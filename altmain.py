...
def cmdlist():
    ...
    pc.printout("wtagged\t\t")
    print("Get a list of users who tagged the target")
...
parser = argparse.ArgumentParser(description='Osintgram is an OSINT tool on Instagram. It offers an interactive shell '
                                             'to perform analysis on an Instagram account of any user by their nickname.')
parser.add_argument('id', type=str, help='username')
parser.add_argument('-C','--cookies', help='clear previous cookies', action="store_true")
parser.add_argument('-j', '--json', help='save commands output as a JSON file', action='store_true')
parser.add_argument('-f', '--file', help='save output in a file', action='store_true')
parser.add_argument('-c', '--command', help='run in single command mode & execute the provided command', action='store')
